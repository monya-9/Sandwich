 ...
 */
export async function fetchUserRecommendations(userId: number, limit: number = 50, baseUrl?: string): Promise<RecoResponse> {
    const isLocalDev = typeof window !== 'undefined' && /localhost:\d+/.test(window.location.host);

    if (isLocalDev) {
        // 로컬 개발에서는 프록시만 사용 (외부 직접 호출 금지: CORS 방지)
        const res = await fetch(`/ext/reco/user/${userId}?limit=${limit}`, { credentials: "omit" });
    ...
    const directUrl = `${AI_BASE}/api/reco/user/${userId}?limit=${limit}`;
