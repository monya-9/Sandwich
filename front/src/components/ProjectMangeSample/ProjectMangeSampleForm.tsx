...
import UploadDoneModal from "./UploadDoneModal";
  ...
  useEffect(() => {
   ...
   useEffect(() => {
      const bootstrapEdit = async () => {
         if (!editOwnerId || !editProjectId) return;
         try {
            const [detail, items] = await Promise.all([
               fetchProjectDetail(editOwnerId, editProjectId).catch(() => null),
               fetchProjectContents(editOwnerId, editProjectId).catch(() => [] as ProjectContentResponseItem[]),
            ]);
            setEditDetail(detail);
            const sorted = (items || []).slice().sort((a, b) => a.order - b.order);
            // PM_META 파싱: bg/gap을 편집 상태에 반영
            try {
              const metaItem = sorted.find(it => it.type === 'TEXT' && typeof it.data === 'string' && it.data.startsWith('<!--PM_META'));
              if (metaItem) {
                const json = metaItem.data.replace(/^<!--PM_META/, '').replace(/-->$/, '');
                const meta = JSON.parse(json || '{}');
                if (meta && typeof meta.bg === 'string' && meta.bg) setBackgroundColor(meta.bg);
                if (meta && typeof meta.gap === 'number' && isFinite(meta.gap)) setContentGapPx(meta.gap);
              }
            } catch {}
            // 메타 항목은 에디터 복원에서 제외
            ...
            if (quill) {
               ...
               // 붙여넣기 후, 저장된 메타(JSON)의 pad/full/padded를 DOM에 재적용
               try {
                 ...
                 const normalize = (u: string) => {
                   try {
                     ...
                     const noQs = noHash.replace(/[?].*$/, '');
                     return noQs.replace(/\/$/, '');
                   } catch { return String(u || ''); }
                 };
                 // 노드 큐 준비(동일 src가 여러 번 있는 경우 순서대로 매칭)
                 const imgMap = new Map<string, HTMLImageElement[]>();
                 const iframeMap = new Map<string, HTMLIFrameElement[]>();
                 Array.from(rootEl.querySelectorAll('img')).forEach(n => {
                   const k = normalize((n as HTMLImageElement).src || n.getAttribute('src') || '');
                   if (!k) return; const arr = imgMap.get(k) || []; arr.push(n as HTMLImageElement); imgMap.set(k, arr);
                 });
                 Array.from(rootEl.querySelectorAll('iframe')).forEach(n => {
                   const k = normalize((n as HTMLIFrameElement).src || n.getAttribute('src') || '');
                   if (!k) return; const arr = iframeMap.get(k) || []; arr.push(n as HTMLIFrameElement); iframeMap.set(k, arr);
                 });
                 const takeNode = <T extends HTMLElement>(map: Map<string, T[]>, src: string): T | undefined => {
                   const key = normalize(src); if (!key) return undefined; const arr = map.get(key) || [];
                   const node = arr.shift(); if (arr.length) map.set(key, arr); else map.delete(key); return node;
                 };
                 const applyMeta = (it: ProjectContentResponseItem) => {
                   try {
                     const obj = JSON.parse(it.data);
                     const src: string = obj?.src || '';
                     if (!src) return;
                     if (it.type === 'IMAGE') {
                       const node = takeNode(imgMap, src) as HTMLImageElement | undefined;
                       if (!node) return;
                       if (obj.padded) { node.classList.add('pm-embed-padded'); node.style.setProperty('--pm-pad', String(obj.pad || 0) + 'px'); } else { node.classList.remove('pm-embed-padded'); node.style.removeProperty('--pm-pad'); }
                       if (obj.full) node.classList.add('pm-embed-full'); else node.classList.remove('pm-embed-full');
                       node.style.marginTop = '0px'; node.style.display = 'block'; node.style.marginLeft = 'auto'; node.style.marginRight = 'auto';
                     }
                     if (it.type === 'VIDEO') {
                       const node = takeNode(iframeMap, src) as HTMLIFrameElement | undefined;
                       if (!node) return;
                       if (obj.padded) { node.classList.add('pm-embed-padded'); (node as HTMLElement).style.setProperty('--pm-pad', String(obj.pad || 0) + 'px'); } else { node.classList.remove('pm-embed-padded'); (node as HTMLElement).style.removeProperty('--pm-pad'); }
                       if (obj.full ?? true) node.classList.add('pm-embed-full'); else node.classList.remove('pm-embed-full');
                       (node as HTMLElement).style.marginTop = '0px'; (node as HTMLElement).style.display = 'block';
                     }
                   } catch {}
                 };
                 contentItems.forEach((it) => applyMeta(it));
               } catch {}
               recalcCounts();
            }
         } catch {}
	...
	const insertEmbedAsBlock = (type: 'image' | 'video', value: string) => {
		...
		setTimeout(() => {
			try {
				...
				// NEW: eligible media gets default padding 200px on insert
				if (isPaddingEligible(el as HTMLElement)) {
					if (!el.classList.contains('pm-embed-padded')) {
						el.classList.add('pm-embed-padded');
						(el as HTMLElement).style.setProperty('--pm-pad', `${DEFAULT_INIT_PAD}px`);
					}
				}
			} catch {}
		}, 0);
   ...
   const [isUploadDoneOpen, setIsUploadDoneOpen] = useState(false);
   ...
   function extractContentItems(): ProjectContentUpsertItem[] {
      const editor = mainQuillRef.current?.getEditor();
      if (!editor) return [];
      const root = editor.root as HTMLElement;
      const blocks: ProjectContentUpsertItem[] = [];
      let order = 0;
      const elems = Array.from(root.children) as HTMLElement[];
      for (const block of elems) {
        // 0) 블록 자체가 미디어인 경우 직접 처리
        const tag = (block.tagName || '').toLowerCase();
        if (tag === 'img') {
          const img = block as unknown as HTMLImageElement;
          if (img.src) {
            let pad = 0; let full = false; let padded = false;
            try {
              const cls = img.classList; full = cls.contains('pm-embed-full'); padded = cls.contains('pm-embed-padded');
              if (padded) {
                const inlineVar = (img as HTMLElement).style.getPropertyValue('--pm-pad');
                const compVar = window.getComputedStyle(img).getPropertyValue('--pm-pad');
                pad = parseInt((inlineVar || compVar || '0').toString().replace('px','')) || 0;
              }
            } catch {}
            blocks.push({ type: 'IMAGE', data: JSON.stringify({ src: img.src, pad, full, padded }), order: order++ });
          }
          continue;
        }
        if (tag === 'iframe') {
          const iframe = block as unknown as HTMLIFrameElement;
          if (iframe.src) {
            let pad = 0; let full = true; let padded = false;
            try {
              const cls = iframe.classList;
              full = cls.contains('pm-embed-full') || true;
              padded = cls.contains('pm-embed-padded');
              if (padded) {
                const inlineVar = (iframe as HTMLElement).style.getPropertyValue('--pm-pad');
                const compVar = window.getComputedStyle(iframe).getPropertyValue('--pm-pad');
                pad = parseInt((inlineVar || compVar || '0').toString().replace('px','')) || 0;
              }
            } catch {}
            blocks.push({ type: 'VIDEO', data: JSON.stringify({ src: iframe.src, pad, full, padded }), order: order++ });
          }
          continue;
        }
        // 1) 먼저 미디어(이미지/영상)를 문서 순서대로 수집
        const mediaNodes = Array.from(block.querySelectorAll('img,iframe')) as Array<HTMLImageElement | HTMLIFrameElement>;
        if (mediaNodes.length > 0) {
          for (const node of mediaNodes) {
            if (node.tagName.toLowerCase() === 'img') {
              ...
              try {
                ...
                full = cls.contains('pm-embed-full');
                padded = cls.contains('pm-embed-padded');
                if (padded) {
                  const inlineVar = (img as HTMLElement).style.getPropertyValue('--pm-pad');
                  const compVar = window.getComputedStyle(img).getPropertyValue('--pm-pad');
          ...
          // 텍스트-only는 따로 처리하지 않음 (보통 미디어 전용 블록)
          continue;
        }
        // 2) 미디어가 없는 블록은 텍스트 또는 내장 동영상 링크 탐지
        const html = (block.innerHTML || '').trim();
        const plain = block.textContent || '';
        // a) YouTube/Vimeo URL이 텍스트에 있으면 VIDEO로 변환 저장
        const urlRegex = /(https?:\/\/[^\s"'<>{}]+)/ig;
        const urls: string[] = [];
        let m: RegExpExecArray | null;
        while ((m = urlRegex.exec(plain)) !== null) { urls.push(m[1]); }
        const embeds: string[] = urls.map(u => normalizeVideoUrl(u) || '').filter(Boolean) as string[];
        if (embeds.length > 0) {
          for (const src of embeds) {
            blocks.push({ type: 'VIDEO', data: JSON.stringify({ src, pad: 0, full: true, padded: false }), order: order++ });
          }
          continue;
        }
        // b) 일반 텍스트만 남으면 TEXT로 저장
        if (html && html.replace(/<br\s*\/>/gi, '').replace(/&nbsp;/g, '').trim().length > 0) {
          blocks.push({ type: 'TEXT', data: block.outerHTML, order: order++ });
        }
      }
      return blocks;
   }
   ...
   async function persistContents(userId: number, projectId: number, isEdit: boolean) {
      ...
      if (withMeta.length > 0) {
          // 보강: 저장 직전, VIDEO/IMAGE data가 순수 URL이면 JSON 메타로 강제 래핑
          const normalized = withMeta.map((it) => {
              if (it.type === 'VIDEO' || it.type === 'IMAGE') {
                 const raw = String(it.data || '');
                 let isJson = false;
                 try { JSON.parse(raw); isJson = true; } catch {}
                 if (!isJson) {
                    const wrapped = { src: raw, pad: 0, padded: false, full: it.type === 'VIDEO' ? true : false } as any;
                    return { ...it, data: JSON.stringify(wrapped) };
                 }
              }
              return it;
           });
          await saveProjectContents(userId, projectId, normalized);
       }
    }
	...
	return (
		<>
            ...
            <ProjectDetailsModal 
               ...
               onCreated={async (createdProjectId, previewUrl) => {
                  ...
                  setLastCreatedProjectId(createdProjectId);
                  setIsUploadDoneOpen(true);
               }}
               libraryImages={detailLibraryImages}
               ...
               onCategoriesChange={setPreviewCategories}
               onCoverChange={(u) => setPreviewCoverUrl(u || null)}
            />
            <ProjectPreviewModal 
               open={isPreviewOpen}
            ...
            />
            <UploadDoneModal
               open={isUploadDoneOpen}
               onClose={() => setIsUploadDoneOpen(false)}
               ownerId={(editOwnerId || Number(localStorage.getItem('userId') || sessionStorage.getItem('userId') || '0') || undefined) as any}
               projectId={(lastCreatedProjectId || editProjectId || undefined) as any}
            />
			</main>
		</div>
		</>
