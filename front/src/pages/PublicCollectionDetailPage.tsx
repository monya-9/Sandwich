    ...
    <div className="w-full flex justify-center">
      ...
      <div className="w-full min-h-screen bg-white font-gmarket px-4 md:px-8 xl:px-14 pb-16">
        <div className="max-w-[1100px] mx-auto pt-20">
          ...
          {loading ? (
            <div className="w-full flex items-center justify-center min-h-[320px] text-black/60">불러오는 중…</div>
          ) : projects.length === 0 ? (
            <div className="w-full flex flex-col items-center justify-center" style={{ minHeight: "calc(100vh - 300px)" }}>
              ...
              <button onClick={() => navigate(`/users/${userIdParam}`)} className="mt-6 h-10 px-5 rounded-full border border-[#D1D5DB] bg-white text-black/80 hover:bg-neutral-50">프로필로 돌아가기</button>
            </div>
          ) : (
