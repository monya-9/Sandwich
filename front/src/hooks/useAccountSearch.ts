...
// 더미 데이터 (함수 외부로 이동)
const dummyAccounts: AccountSearchResult[] = [
  { 
    id: 1, 
    nickname: '이병건', 
...
export const useAccountSearch = () => {
  ...
  const performSearch = useCallback(async (params: AccountSearchParams) => {
    ...
    // 임시로 더미 데이터 사용 (API 문제 해결 전까지)
    console.log('계정 검색 (더미 데이터 사용):', searchParams);
    
    // 검색어에 따라 필터링된 더미 데이터 반환
    const filteredAccounts = dummyAccounts.filter(account => 
      account.nickname.toLowerCase().includes(searchParams.q.toLowerCase()) ||
      account.email?.toLowerCase().includes(searchParams.q.toLowerCase()) ||
      account.position?.name.toLowerCase().includes(searchParams.q.toLowerCase())
    );
    ...
    showToast('success', `${filteredAccounts.length}개의 계정을 찾았습니다.`);
  }, []);
